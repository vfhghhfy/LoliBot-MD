...
// إذا كان هناك participantAlt صالح، استخدمه
...
m.pp = await conn.profilePictureUrl(m.who, 'image').catch(_ => 'https://files.catbox.moe/gq9ja7.jpg');
...
m.download = async () => {
    ...
    if (!messageContent) throw new Error('لم يتم العثور على محتوى للتحميل');
    const type = Object.keys(messageContent)[0];
...
/*  newsletterJid: (() => {
  ...
  conn.reply = async (chatId, text, quoted = null, options = {}) => {
    const contextInfo = {
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid: "120363323732849137@newsletter",
       newsletterName: "دزاري يريد ان ينتحر"
      }
    };
...
conn.sendFile = async function (jid, path, filename = '', caption = '', quoted = null, ptt = false, options = {}) {
  ...
  if (Buffer.isBuffer(path)) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`خطأ HTTP ${res.status}: ${res.statusText}`);
...
conn.sendAlbumMessage = async function (jid, medias = [], caption = '', quoted = null) {
  if (!Array.isArray(medias) || medias.length === 0) {
    throw new Error("لم يتم تقديم وسائط صالحة.");
  }

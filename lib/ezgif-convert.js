...
async function convert(fields) {
    ...
    if (!type) throw new Error(`نوع التحويل غير صالح "${fields?.type}"`);
    let form = new FormData();

    if (fields?.file) {
        if (!fields.filename) throw new Error(`يجب تقديم اسم الملف لرفع الملفات. (مع الامتداد)`);
        form.append('new-image', fields.file, {
            filename: fields.filename,
        });
    } else if (fields?.url) {
        form.append('new-image-url', fields.url);
    } else throw new Error('إما حقل file أو url مطلوب.');
    ...
    if (type.req_params) {
        type.req_params.forEach(e => {
            if (!org_keys.includes(e)) throw new Error(`"${e}" معامل مطلوب.`);
        });
    }
    if (type.either_params.length) {
        ...
        if (!check) throw new Error(`يجب تقديم أحد هذه المعاملات: ${type.either_params.join(', ')}`);
    }
        ...
        if (error.response) {
            throw new Error(
                JSON.stringify({
                        ...
                        data: error.response.data.length ? error.response.data : "حاول مرة أخرى. إذا استمرت المشكلة، قم بالإبلاغ للمطور.",
                    },
                    null,
                    4
                )
            ...
            throw new Error("عفواً، حدث خطأ غير معروف! :(");
        }
    });

    let redir = String(link?.request?.res?.responseUrl);
    if (!redir) throw new Error(`عفواً! حدث خطأ غير معروف!`);
    ...
    if (img_url.includes('undefined')) throw new Error(`حدث خطأ غير معروف هنا... يرجى الإبلاغ للمطور`);
    return img_url;
}
...
async function overlay(fields) {
    ...
    if (!redir_over) throw new Error(`عفواً! حدث خطأ غير معروف!`);
...
async function render(fields) {
    ...
    let form = new FormData();
    if (!type) throw new Error(`نوع التصيير غير صالح "${fields?.type}"`);
    let default_ = {
        delay: 20,
        dfrom: 1,
        dto: 5,
    ...
    for (let i = 0; i < fields.files.length; i++) {
        if (!fields.files[i].data) throw new Error(`لم يتم تقديم بيانات الملف لـ files[${i}]`);
        if (!fields.files[i].name) throw new Error(`لم يتم تقديم اسم الملف لـ files[${i}]`);
